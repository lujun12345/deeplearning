...
class XSleepNet(nn.Module):
    def __init__(self, config):
        ...
        # 5. 门控层
        self.align_seq = nn.Linear(2 * self.seq_nhidden2, 2 * self.deep_nhidden)

        self.gate = nn.Sequential(
            nn.Linear(2 * self.deep_nhidden * 2, 2 * self.deep_nhidden),
            nn.Sigmoid()
        )

        self.fusion_output_layers = nn.ModuleList([
            nn.Linear(2 * self.deep_nhidden, self.nclass)  # 原代码输入维度错误，这里修正
    ...
    def forward(self, x):
        ...
        for i in range(self.epoch_seq_len):
            ...
            feat_deep = deep_rnn_out[:, i, :]
            feat_seq = seq_rnn_out[:, i, :]

            feat_seq_aligned = self.align_seq(feat_seq)  # [B, 768]

            g = self.gate(torch.cat([feat_deep, feat_seq_aligned], dim=1))
            fused_feat = g * feat_deep + (1 - g) * feat_seq_aligned

            fused_score = self.fusion_output_layers[i](fused_feat)
            fused_scores.append(fused_score)

        fused_scores = torch.stack(fused_scores, dim=1).transpose(1,2)
        assert fused_scores.shape[0] == batch_size, f"fused_scores batch错误: {fused_scores.shape[0]} != {batch_size}"
    ...
    def compute_loss(self, outputs, y):
        ...
        for i in range(self.epoch_seq_len):
            ...
            # fused分支损失
            fused_score_i = fused_scores[:, :, i]
            fused_loss_i = F.cross_entropy(fused_score_i, y_i)
            fused_loss += fused_loss_i

        # 平均到每个epoch
        deep_loss /= self.epoch_seq_len
        seq_loss /= self.epoch_seq_len
        joint_loss /= self.epoch_seq_len
        fused_loss /= self.epoch_seq_len
        ...
        # 4. 总损失
        total_loss = fused_loss + self.l2_reg_lambda * l2_loss
