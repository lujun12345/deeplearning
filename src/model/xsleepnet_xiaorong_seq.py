...
class XSleepNet(nn.Module):
    def __init__(self, config):
        ...
        self.fusion_output_layers = nn.ModuleList([
            # 输入维度 = Deep分支特征 + Seq分支特征 + Joint分支特征
            nn.Linear(2 * self.deep_nhidden + 2 * self.seq_nhidden2 + (2 * self.deep_nhidden + 2 * self.seq_nhidden2),
                      self.nclass)
    ...
    def _build_seq_sleepnet_branch(self):
        ...
        self.channel_interact = nn.Parameter(torch.eye(self.nchannel))  # 通道交互矩阵
        ...
        if self.nchannel == 4 :
            ...
            self.filter_bank = nn.ParameterList([self.Weeg1, self.Weeg2, self.Weog, self.Wemg])
            ...
            self.Wemg = nn.Parameter(torch.randn(self.seq_ndim, self.seq_nfilter))
            self.filter_bank = nn.ParameterList([self.Weeg1, self.Weeg2, self.Weog1, self.Weog2, self.Wemg])

        elif self.nchannel == 3 :
            self.Weeg = nn.Parameter(torch.randn(self.seq_ndim, self.seq_nfilter))
            self.Weog = nn.Parameter(torch.randn(self.seq_ndim, self.seq_nfilter))
            self.Wemg = nn.Parameter(torch.randn(self.seq_ndim, self.seq_nfilter))
            self.filter_bank = nn.ParameterList([self.Weeg, self.Weog, self.Wemg])
        else:
    ...
    def _seq_sleepnet_forward(self, x2):
        ...
        channel_weight = F.softmax(self.channel_interact, dim=1)  # [C, C]
        x2 = torch.einsum('b t1 t2 d c, c c2 -> b t1 t2 d c2', x2, channel_weight)
