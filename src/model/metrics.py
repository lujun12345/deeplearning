...
def overall_accuracy(output, target):
    ...
    return metrics.accuracy_score(target.flatten(), output.flatten())
...
def balanced_accuracy(output, target):
    ...
    return metrics.balanced_accuracy_score(target.flatten(), output.flatten())
...
def kappa(output, target):
    ...
    return metrics.cohen_kappa_score(output.flatten(),target.flatten(),labels=[0, 1, 2, 3, 4])
...
def precision(output, target):
    ...
    return metrics.precision_score(target.flatten(), output.flatten(),
...
def recall(output, target):
    ...
    return metrics.recall_score(target.flatten(), output.flatten(),
                                labels=[0, 1, 2, 3, 4], average='macro')
...
def f1(output, target):
    ...
    return metrics.f1_score(target.flatten(), output.flatten(),
                            labels=[0, 1, 2, 3, 4], average='macro')
...
def overall_f1(output, target):
    return metrics.f1_score(target.data.cpu().numpy().flatten(), output.data.cpu().numpy().argmax(1).flatten(), labels=[0, 1, 2, 3, 4], average='micro')
