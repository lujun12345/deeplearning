...
loss = df["overall_accuracy"].values
val_loss = df["val_overall_accuracy"].values
...
plt.plot(epochs, loss, color="#a0c4ff", alpha=0.4, linestyle="--", linewidth=1, label="原始训练overall_accuracy")
plt.plot(epochs, val_loss, color="#ffadad", alpha=0.4, linestyle="--", linewidth=1, label="原始验证overall_accuracy")
...
plt.plot(epochs_smooth, loss_smooth, color="#3d85c6", linewidth=3, marker="o", markersize=4, label=f"训练overall_accuracy（{SMOOTH_WINDOW}epoch平滑）")
plt.plot(epochs_smooth, val_loss_smooth, color="#ff6b6b", linewidth=3, marker="s", markersize=4, label=f"验证overall_accuracy（{SMOOTH_WINDOW}epoch平滑）")

# 标题与标签
# plt.title(f"训练/验证损失曲线（{SMOOTH_WINDOW}个epoch平滑）", fontsize=16, pad=15)
plt.title(f"训练/验证准确率曲线（{SMOOTH_WINDOW}个epoch平滑）", fontsize=16, pad=15)
...
# 保存图片
plt.savefig("accuracy_curve_smooth.png", dpi=300, bbox_inches="tight")
plt.close()

print("✅ 绘图完成！已保存为：accuracy_curve_smooth.png")
